...
export const handlers = [
  ...
  // Error-State Mocks for Login Flows
  http.post('/api/auth/login-error/invalid-credentials', async ({ request }) => {
    await delay(100);
    return HttpResponse.json(
      {
        success: false,
  ...
  http.post('/api/auth/login-error/expired-account', async ({ request }) => {
    await delay(100);
    return HttpResponse.json(
      {
        success: false,
  ...
  // Error-State Mocks for Onboarding Flows
  http.patch('/api/auth/user/:userId/onboarding-error/server-error', async ({ params }) => {
    await delay(100);
    return HttpResponse.json(
      {
        success: false,
        error: {
          ...
          message: 'Failed to save onboarding preferences. Please try again.',
          details: {
            errorId: 'onboarding-save-failed-12345',
          },
        },
      },
      { status: 500 }
  ...
  http.patch('/api/auth/user/:userId/onboarding-error/timeout', async ({ params }) => {
    await delay(6000); // 6 second timeout
    return HttpResponse.json(
      {
        success: false,
        error: {
  ...
  http.patch('/api/auth/user/:userId/onboarding-error/network-error', async ({ params }) => {
    await delay(100);
    return HttpResponse.json(
      {
        success: false,
