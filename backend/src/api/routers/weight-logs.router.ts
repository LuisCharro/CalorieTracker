...
router.get('/progress', async (req, res) => {
  ...
  try {
    ...
    // Get trend data for 7 days
    const trend7dResult = await query(
      `SELECT DATE(logged_at) as date, weight_value as weight
       FROM weight_logs
       WHERE user_id = $1 
    ...
    const trend30dResult = await query(
      `SELECT DATE(logged_at) as date, weight_value as weight
       FROM weight_logs
       WHERE user_id = $1 
    ...
    // Format trend data
    const formatTrend = (rows: { date: Date; weight: string }[]) => 
      rows.map(row => ({
        date: row.date instanceof Date ? row.date.toISOString().split('T')[0] : String(row.date).split('T')[0],
        weight: parseFloat(row.weight),
      }));

    const trend7d = formatTrend(trend7dResult.rows);
    const trend30d = formatTrend(trend30dResult.rows);
