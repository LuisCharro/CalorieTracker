...
## Backend tasks
- [ ] Confirm `calorietracker_test` schema mirrors production migrations (create a script `scripts/ensure_test_db.sh` or extend `dev-scripts/restart-stack.sh` to `createdb -w calorietracker_test`).
- [ ] Add integration coverage for onboarding endpoints (`POST /signup`, `/onboarding/goals`, `/onboarding/preferences`) to smoke matrix.
...
## Frontend tasks
- [ ] Tighten the onboarding screens (goal selection, preference toggles, summary) to match the backend contract from the plan document; ensure the UI assertions reference the same fields.
...
- [ ] Generate a `plan/02_architecture_gdpr/30_FRONTEND_COMPONENT_INVENTORY.md` (new doc) to list active screens/components and their API dependencies; use it to spot missing coverage.
